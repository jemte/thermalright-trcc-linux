...
class TestInstanceDiscovery:
    def test_load_local_themes_with_dir(self, svc: ThemeService, tmp_path: Path) -> None:
    ...
    def test_load_cloud_themes_with_dir(self, svc: ThemeService, tmp_path: Path) -> None:
        _make_cloud_dir(tmp_path, ["a001.mp4"])
        svc.set_directories(web_dir=tmp_path)
    ...
    def test_load_cloud_themes_uses_category(self, svc: ThemeService, tmp_path: Path) -> None:
        _make_cloud_dir(tmp_path, ["a001.mp4", "b002.mp4"])
        svc.set_directories(web_dir=tmp_path)
        svc.set_category("a")
    ...
    def test_load_local_themes_uses_filter(self, svc: ThemeService, tmp_path: Path) -> None:
        _make_theme_dir(tmp_path, "Normal")
...
class TestLoadReferenceBased:
    def test_reference_video_background(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
    ...
    def test_reference_with_mask(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        mask_dir = _make_theme_dir(
            tmp_path, "MaskSrc", has_mask=True, has_bg=False, has_preview=False
        )
    ...
    def test_reference_bg_not_exists(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        td = _make_theme_dir(
            tmp_path,
            "RefMissing",
            has_json=True,
    ...
    def test_reference_no_bg_no_mask(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        td = _make_theme_dir(
            tmp_path,
            "RefEmpty",
            has_json=True,
    ...
    def test_reference_zt_background(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
...
class TestLoadCopyBased:
    def test_static_bg(self, tmp_path: Path, lcd_size: tuple[int, int], big_image: Any) -> None:
    ...
    def test_zt_animation(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        td = _make_theme_dir(tmp_path, "Animated", has_bg=False, has_zt=True, has_preview=True)
        work = tmp_path / "work"
        theme = ThemeInfo.from_directory(td)
    ...
    def test_mp4_animation_in_dir(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        td = _make_theme_dir(tmp_path, "Mp4Theme", has_bg=True, has_mp4=True)
        work = tmp_path / "work"
        theme = ThemeInfo.from_directory(td)
    ...
    def test_mask_only_theme(
        ...
        td = _make_theme_dir(tmp_path, "MaskOnly", has_bg=False, has_mask=True, has_preview=True)
        work = tmp_path / "work"
        theme = ThemeInfo.from_directory(td)
    ...
    def test_anim_file_from_dc_options(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
    ...
    def test_mask_loaded_from_working_dir(
        ...
        td = _make_theme_dir(tmp_path, "WithMask", has_bg=True, has_mask=True)
        work = tmp_path / "work"
        theme = ThemeInfo.from_directory(td)
    ...
    def test_copy_based_mask_with_dc(
        ...
        td = _make_theme_dir(tmp_path, "MaskDc", has_bg=True, has_mask=True, has_dc=True)
        work = tmp_path / "work"
        theme = ThemeInfo.from_directory(td)
...
class TestSave:
    ...
    def test_no_background_fails(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        ok, msg = ThemeService.save(
            "Fail",
            tmp_path,
            lcd_size,
    ...
    def test_exception_returns_false(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        bad_bg = MagicMock()
...
class TestExportImport:
    ...
    def test_export_failure(self, tmp_path: Path) -> None:
        ...
        svc = ThemeService(export_theme_fn=mock_export)
        ok, msg = svc.export_tr(tmp_path / "bad", tmp_path / "out.tr")
    ...
    def test_import_failure(self, tmp_path: Path, lcd_size: tuple[int, int]) -> None:
        mock_import = MagicMock(side_effect=ValueError("bad header"))
        svc = ThemeService(import_theme_fn=mock_import)
        ok, msg = svc.import_tr(tmp_path / "bad.tr", tmp_path, lcd_size)
    ...
    def test_import_resolution_mismatch_warning(
        ...
        mismatched_theme = ThemeInfo(name="big", resolution=(480, 480))
...
class TestParseMaskPosition:
    ...
    def test_small_mask_dc_not_exists_centers(self, tmp_path: Path) -> None:
        svc = ThemeService()
        pos = svc._parse_mask_position(tmp_path / "nonexistent.dc", 100, 100, 320, 320)
        assert pos == (110, 110)
...
class TestLoadMaskInto:
    def test_success(self, tmp_path: Path) -> None:
        mask_dir = _make_theme_dir(tmp_path, "Mask", has_mask=True, has_bg=False, has_preview=False)
    ...
    def test_exception_logged(self, tmp_path: Path) -> None:
        ...
        with (
            patch("trcc.services.theme.ImageService._r", return_value=mock_renderer),
            patch("trcc.services.theme.log") as mock_log,
        ):
            svc._load_mask_into(data, td, 320, 320)
