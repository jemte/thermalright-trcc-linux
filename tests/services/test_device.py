...
class TestSelection:
    def test_select(self):
        svc = _make_service()
        dev = DeviceInfo(name="test", path="/dev/sg0", vid=1, pid=2, device_index=0)
        svc.select(dev)
...
def test_every_known_device_in_registry(vid_pid, entry):
    ...
    assert entry.implementation, f"{vid_pid}: DeviceEntry has no implementation name"


@pytest.mark.parametrize(
    "vid_pid,entry", list(SCSI_DEVICES.items()), ids=[f"{v:04X}:{p:04X}" for v, p in SCSI_DEVICES]
)
...
@pytest.mark.parametrize(
    "fbl,profile", list(FBL_PROFILES.items()), ids=[str(f) for f in FBL_PROFILES]
)
