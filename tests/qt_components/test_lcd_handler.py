...
class TestApplyDeviceConfig:
    ...
    @patch("trcc.qt_components.lcd_handler.Settings")
    def test_resolution_dispatches_set_resolution_command(self, mock_settings):
        """SetResolutionCommand always dispatched — seeds per-device DisplayService dims."""
        ...
        # SetResolutionCommand always dispatched so DisplayService._width/_height is seeded
    ...
    def test_widgets_always_updated(self, mock_settings):
        ...
        h = _make_handler()
        h.apply_device_config(self._device(), 320, 320)
        # Widget updates are unconditional
...
class TestThemeSelection:
    ...
    def test_select_theme_persists_path(self, mock_ti, mock_settings):
        ...
        mock_settings.save_device_setting.assert_any_call("dev0", "theme_path", "/themes/TestTheme")
...
class TestMask:
    ...
    def test_apply_mask_with_path(self, mock_settings):
        ...
        h._lcd.load_mask_standalone.return_value = {"success": True, "image": MagicMock()}
        h.apply_mask(mask_info)
        h._lcd.load_mask_standalone.assert_called_once()
        h._w["preview"].set_image.assert_called_once()
        mock_settings.save_device_setting.assert_called_with("dev0", "mask_path", "/masks/01")
    ...
    @patch("trcc.qt_components.lcd_handler.Settings")
    @patch("trcc.qt_components.lcd_handler.ThemeInfo")
    def test_auto_load_skipped_when_saved_theme_exists(self, mock_ti, mock_settings):
        """_update_theme_directories must not auto-load Theme1 when a saved theme_path exists.

        RestoreLastThemeCommand owns session restore. Auto-loading here would
        ...
        mock_ti.from_directory.return_value = MagicMock()
        mock_settings.device_config_key.return_value = "1"
        # Simulate a saved theme_path in config — get_device_config is called
        # multiple times (once for each restore step + _update_theme_directories).
        mock_settings.get_device_config.return_value = {
            "theme_path": "/home/user/.trcc-user/theme1600720/Custom_Theme6",
        }
        lcd = _make_lcd()
        lcd.current_image = None  # no image loaded yet (startup state)

        with tempfile.TemporaryDirectory() as tmp:
            # Create a fake theme dir so _update_theme_directories finds it
            theme_dir = _Path(tmp) / "Theme1"
            theme_dir.mkdir()
            ...
            with _patch("trcc.core.models.ThemeDir") as mock_td:
                mock_td.for_resolution.return_value = td
                h = _make_handler(lcd=lcd)
                h.apply_device_config(MagicMock(device_index=1, vid=0x87AD, pid=0x70DB), 1600, 720)

        # _select_theme_from_path must NOT have been called for Theme1 — the saved
        # Custom_Theme6 must not be overwritten by the auto-load fallback.
        # RestoreLastThemeCommand handles the actual restore.
        for call in lcd.theme.select.call_args_list:
            theme = call.args[0] if call.args else call.kwargs.get("theme")
            if theme:
                assert "Theme1" not in str(getattr(theme, "name", "")), (
                    "_update_theme_directories auto-loaded Theme1, overwriting saved theme"
                )
    ...
    def test_restore_dispatches_restore_last_theme_command(self, mock_settings):
        ...
        h = _make_handler(lcd=lcd)
        h.apply_device_config(MagicMock(device_index=0, vid=0x0402, pid=0x3922), 320, 320)
...
class TestThemeIO:
    ...
    def test_import_config_success_reloads(self, mock_conf):
        ...
        td.exists.return_value = True
        mock_conf.theme_dir = td
        h._lcd.theme.import_config.return_value = {"success": True, "message": "Imported"}
