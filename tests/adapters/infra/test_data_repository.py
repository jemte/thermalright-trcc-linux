...
class TestExtract7z(unittest.TestCase):
    ...
    def test_7z_cli_success(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch(
                "trcc.adapters.infra.data_repository.subprocess.run", return_value=mock_result
            ):
    ...
    def test_7z_not_found(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch(
                "trcc.adapters.infra.data_repository.subprocess.run", side_effect=FileNotFoundError
            ):
                result = DataManager.extract_7z(archive, target)
...
class TestEnsureThemesExtracted(unittest.TestCase):
    ...
    def test_already_present(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                patch.object(DataManager, "_data_dir", staticmethod(lambda: d)),
                patch.object(
                    DataManager, "_data_dir", staticmethod(lambda: os.path.join(d, "user"))
                ),
            ):
...
class TestEnsureAll(unittest.TestCase):
    ...
    def test_square_only_installs_native(self):
        ...
        calls = []
        with (
            patch.object(DataManager, "is_resolution_installed", return_value=False),
            ...
            patch.object(
                DataManager, "ensure_web_masks", side_effect=lambda w, h: calls.append((w, h))
            ),
        ):
    ...
    def test_non_square_installs_both_orientations(self):
        ...
        with (
            ...
            patch.object(DataManager, "ensure_themes"),
            patch.object(
                DataManager, "ensure_web", side_effect=lambda w, h: web_calls.append((w, h))
            ),
            patch.object(
                DataManager, "ensure_web_masks", side_effect=lambda w, h: mask_calls.append((w, h))
            ),
        ):
    ...
    def test_skips_if_already_installed(self):
        ...
        with (
            patch.object(DataManager, "is_resolution_installed", return_value=True),
...
class TestFindPkgDataDir(unittest.TestCase):
    ...
    def test_returns_this_dir_data_as_fallback(self):
        ...
        with (
            patch("trcc.adapters.infra.data_repository._THIS_DIR", "/fake/src/trcc"),
