...
class TestAK120Display:
    ...
    def test_cpu_phase(self):
        mask = self.d.compute_mask(
            HardwareMetrics(cpu_temp=50, cpu_percent=75, cpu_power=120), 0, "C"
        )
    ...
    def test_gpu_phase(self):
        mask = self.d.compute_mask(
            HardwareMetrics(gpu_temp=60, gpu_usage=90, gpu_power=200), 1, "C"
        )
...
class TestLEDServiceSegmentMode:
    ...
    def test_update_segment_mask_style_5(self):
        svc = self._make_service(5)
        svc.update_metrics(
            HardwareMetrics(cpu_temp=65, cpu_power=100, cpu_freq=2000, cpu_percent=50)
        )
        svc._update_segment_mask()
        assert svc._segment_mask is not None
