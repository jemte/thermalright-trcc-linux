...
class TestScanAndSelect:
    ...
    def test_explicit_path_match(self):
        ...
        with patch.object(svc, "detect"), patch.object(svc, "_discover_resolution"):
    ...
    def test_discover_called_when_selected(self):
        dev = _make_detected_device()
        svc = self._make_svc([dev])
        with patch.object(svc, "detect"), patch.object(svc, "_discover_resolution") as mock_disc:
            svc.scan_and_select()
...
class TestDiscoverResolution:
    ...
    def test_already_known_resolution_is_noop(self):
        ...
        with patch(
            "trcc.adapters.device.factory.DeviceProtocolFactory.get_protocol"
        ) as mock_factory:
    ...
    def test_handshake_sets_resolution(self):
        ...
        with patch(
            "trcc.adapters.device.factory.DeviceProtocolFactory.get_protocol", return_value=protocol
        ):
            discover_resolution(dev)
...
class TestProbe:
    ...
    def test_hid_led_success(self):
        ...
        dev = _make_detected_device(implementation="hid_led", vid=0x0416, pid=0x8001)
    ...
    def test_hid_type2_success_with_serial(self):
        ...
        with (
            patch("trcc.adapters.device.factory.DeviceProtocolFactory") as mock_factory_cls,
    ...
    def test_bulk_success(self):
        ...
        with patch(
            "trcc.adapters.device.factory.DeviceProtocolFactory.create_protocol",
...
class TestFormat:
    ...
    def test_probe_with_resolution(self):
        ...
        dev = _make_detected_device()
        with patch("trcc.cli._device._probe", return_value={"resolution": (360, 360)}):
            line = _format(dev, probe=True)
    ...
    def test_probe_with_serial_truncated_to_16(self):
        ...
        dev = _make_detected_device()
        with patch(
            "trcc.cli._device._probe", return_value={"serial": "ABCDEFGHIJKLMNOPQRSTUVWXYZ"}
        ):
            line = _format(dev, probe=True)
...
class TestDetect:
    ...
    def test_show_all_lists_all_devices(self, capsys):
        ...
        with (
            ...
            result = detect(
                show_all=True, detect_fn=lambda: [dev1, dev2], platform_setup=mock_setup
            )
