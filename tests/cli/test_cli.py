...
class TestMainEntryPoint(unittest.TestCase):
    ...
    def test_detect_all_flag(self):
        ...
        with (
            patch("sys.argv", ["trcc", "detect", "--all"]),
            patch("trcc.cli._device.detect", return_value=0) as mock_detect,
        ):
            main()
    ...
    def test_color_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "color", "ff0000"]),
    ...
    def test_gui_dispatches(self):
        ...
        with patch("sys.argv", ["trcc", "gui"]), patch("trcc.cli.gui", return_value=0) as mock_gui:
            main()
    ...
    def test_download_list(self):
        ...
        with (
            patch("sys.argv", ["trcc", "download", "--list"]),
            patch("trcc.cli._system.download_themes", return_value=0) as mock_dl,
        ):
            main()
            mock_dl.assert_called_once_with(pack=None, show_list=True, force=False, show_info=False)

    def test_download_pack(self):
        with (
            patch("sys.argv", ["trcc", "download", "themes-320", "--force"]),
            patch("trcc.cli._system.download_themes", return_value=0) as mock_dl,
        ):
            main()
...
class TestNewCommandDispatch(unittest.TestCase):
    ...
    def test_brightness_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "brightness", "2"]),
            patch("trcc.cli._display.set_brightness", return_value=0) as mock,
        ):
            main()
    ...
    def test_rotation_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "rotation", "90"]),
            patch("trcc.cli._display.set_rotation", return_value=0) as mock,
        ):
            main()
    ...
    def test_theme_list_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "theme-list"]),
            patch("trcc.cli._theme.list_themes", return_value=0) as mock,
        ):
            main()
    ...
    def test_theme_load_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "theme-load", "myTheme"]),
            patch("trcc.cli._theme.load_theme", return_value=0) as mock,
        ):
            main()
    ...
    def test_led_color_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "led-color", "ff0000"]),
            patch("trcc.cli._led.set_color", return_value=0) as mock,
        ):
            main()
    ...
    def test_led_mode_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "led-mode", "rainbow"]),
            patch("trcc.cli._led.set_mode", return_value=0) as mock,
        ):
            main()
    ...
    def test_led_brightness_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "led-brightness", "50"]),
            patch("trcc.cli._led.set_led_brightness", return_value=0) as mock,
        ):
            main()
    ...
    def test_led_off_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "led-off"]),
            patch("trcc.cli._led.led_off", return_value=0) as mock,
        ):
            main()
    ...
    def test_screencast_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "screencast"]),
            patch("trcc.cli._display.screencast", return_value=0) as mock,
        ):
            main()
    ...
    def test_mask_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "mask", "/tmp/m.png"]),
            patch("trcc.cli._display.load_mask", return_value=0) as mock,
        ):
            main()
    ...
    def test_overlay_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "overlay", "/tmp/dc"]),
            patch("trcc.cli._display.render_overlay", return_value=0) as mock,
        ):
            main()
            mock.assert_called_once_with(
    ...
    def test_theme_save_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "theme-save", "MyTheme"]),
            patch("trcc.cli._theme.save_theme", return_value=0) as mock,
        ):
            main()
            mock.assert_called_once()
    ...
    def test_theme_export_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "theme-export", "Foo", "/tmp/out.tr"]),
            patch("trcc.cli._theme.export_theme", return_value=0) as mock,
        ):
            main()
    ...
    def test_theme_import_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "theme-import", "/tmp/t.tr"]),
            patch("trcc.cli._theme.import_theme", return_value=0) as mock,
        ):
            main()
    ...
    def test_led_sensor_dispatches(self):
        ...
        with (
            patch("sys.argv", ["trcc", "led-sensor", "cpu"]),
            patch("trcc.cli._led.set_sensor_source", return_value=0) as mock,
        ):
            main()
...
class TestTestDisplay(unittest.TestCase):
    ...
    def test_display_success(self):
        ...
        svc = _mock_service()
        with patch("trcc.cli._device._get_service", return_value=svc), patch("time.sleep"):
    ...
    def test_display_error(self):
        """Exception returns 1."""
        with patch("trcc.cli._device._get_service", side_effect=RuntimeError("no device")):
...
class TestScreencast(unittest.TestCase):
    ...
    def _mock_builder(self):
        ...
        builder = MagicMock()
        builder.build_setup.return_value.get_screencast_capture.return_value = ("x11grab", ":0", [])
        return builder
...
class TestMaskClear(unittest.TestCase):
    ...
    def test_mask_clear_dispatches_to_send_color(self):
        ...
        with (
            patch("sys.argv", ["trcc", "mask", "--clear"]),
            patch("trcc.cli._display.send_color", return_value=0) as mock,
        ):
            main()
...
class TestHidDebug(unittest.TestCase):
    ...
    def test_hid_device_import_error(self):
        ...
        with patch(
            "trcc.adapters.device.factory.LedProtocol",
    ...
    def test_dispatch_hid_debug(self):
        ...
        with (
            patch("trcc.cli._diag.hid_debug", return_value=0) as mock_fn,
...
class TestLedDebug(unittest.TestCase):
    ...
    def test_exception_returns_1(self):
        with patch("trcc.adapters.device.factory.LedProtocol", side_effect=Exception("fail")):
            result = led_debug()
        self.assertEqual(result, 1)
    ...
    def test_dispatch_led_debug(self):
        ...
        with (
            patch("trcc.cli._diag.led_debug", return_value=0) as mock_fn,
...
class TestSettingsHelpers(unittest.TestCase):
    ...
    def test_set_and_get_selected(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("trcc.conf.CONFIG_PATH", config_path), patch("trcc.conf.CONFIG_DIR", tmp):
                from trcc.conf import Settings
    ...
    def test_set_with_corrupt_existing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(path, "w") as f:
                f.write("{bad")
            with patch("trcc.conf.CONFIG_PATH", path), patch("trcc.conf.CONFIG_DIR", tmp):
                from trcc.conf import Settings
