...
- **Issue #87 — Bazzite/Python 3.14 typer crash in `sudo_reexec`**: `adapters/system/linux/setup.py`'s `sudo_reexec` re-runs `sys.executable -m trcc.cli subcommand` — this loads full typer CLI which crashes on Python 3.14 (`TypeError: type 'Choice' is not subscriptable` in `class TyperChoice(click.Choice[ParamTypeValue])`). Fix: `sudo_reexec` must bypass `trcc.cli` entirely — call a minimal Python script that only imports the specific setup function needed. PENDING.
...
- **First install auto-load (v9.2.1)**: `EnsureDataCommand` extracts theme archives in a background thread AFTER `_restore_theme` runs (no themes exist yet at that point). When extraction finishes, `lcd.notify_data_ready()` → `on_data_ready` → `_update_theme_directories()`. That method checks `current_image is None` — if so, auto-loads the first local theme with `overlay_config=True` so mask + metrics apply immediately. Hook point: `_update_theme_directories` in `lcd_handler.py`.
...
- **No Workarounds** — when something breaks, find the real root cause and fix it. No runtime fallbacks that silently degrade behaviour, no environment sniffing to paper over broken configs, no dual code paths to accommodate one broken environment. Professional hexagonal/SOLID code is correct by design — if a dependency is missing or misconfigured, fail loudly with a clear error. A hack that works in one environment but silently degrades in another is worse than a clean failure.
