...
class LyDevice(BulkFrameDevice, FrameDevice):
    ...
    def handshake(self) -> HandshakeResult:
        ...
        log.debug("Response hex (first 48): %s", " ".join(f"{b:02x}" for b in resp[:48]))

        # Validate: C# checks array[0]==3 && array[1]==0xFF && array[8]==1
        if len(resp) < 37 or resp[0] != 3 or resp[1] != 0xFF or resp[8] != 1:
            log.warning(
                "LY handshake validation failed: [0]=%s [1]=%s [8]=%s",
        ...
        log.info(
            "LY handshake OK: PM=%d, SUB=%d, FBL=%d, resolution=%s, pid=0x%04x",
            self.pm,
            self.sub_type,
            fbl,
            resolution,
    ...
    def send_frame(self, image_data: bytes) -> bool:
        ...
        try:
            ...
            log.exception("LY frame send failed (%d bytes, %d chunks)", total_size, num_chunks)
            return False
