...
class ScsiDevice(FrameDevice):
    ...
    def _get_frame_chunks(width: int, height: int) -> list:
        ...
        """
        pixels = width * height
        chunk_size = _CHUNK_SIZE_SMALL if pixels <= _SMALL_DISPLAY_PIXELS else _CHUNK_SIZE_LARGE
        ...
        if _sg_io_available is not False:
            try:
                ...
                if _sg_io_available is None:
                    log.warning(
                        "SG_IO read failed (%s: %s), falling back to sg_raw", type(e).__name__, e
                    )
    ...
    def _scsi_write(dev: str, header: bytes, data: bytes) -> bool:
        ...
        if _sg_io_available is not False:
            try:
                ...
                if _sg_io_available is None:
                    log.warning(
                        "SG_IO ioctl failed (%s: %s), falling back to sg_raw", type(e).__name__, e
                    )
                    _sg_io_available = False
    ...
    def _init_device(dev: str) -> tuple[int, bytes]:
        ...
        for attempt in range(_BOOT_MAX_RETRIES):
            ...
            if len(response) >= 8 and response[4:8] == _BOOT_SIGNATURE:
                log.info(
                    "Device %s still booting (attempt %d/%d), waiting %.0fs...",
                    dev,
                    attempt + 1,
                    _BOOT_MAX_RETRIES,
                    _BOOT_WAIT_SECONDS,
    ...
    def _send_boot_animation(
        ...
        if n == 0 or n >= _ANIM_MAX_FRAMES:
            log.warning(
                "Boot animation frame count %d out of range (1-%d)", n, _ANIM_MAX_FRAMES - 1
            )
            return False
        ...
        if not ScsiDevice._scsi_write(dev, header, compressed):
            ...
            return False
        log.info(
            "Boot animation: sent first frame (%d bytes compressed, %d frames total)",
            len(compressed),
            n,
        )
