...
class ThemeInfo:
    ...
    def from_directory(cls, path: Path, resolution: Tuple[int, int] = (320, 320)) -> "ThemeInfo":
        ...
        return cls(
            ...
            thumbnail_path=td.preview
            if td.preview.exists()
            else (td.bg if td.bg.exists() else None),
...
class DetectedDevice:
    ...
    button_image: str = "A1TARAN ARMS"  # Button image prefix (without .png)
...
BULK_DEVICES: dict[tuple[int, int], DeviceEntry] = {
    ...
    (0x87AD, 0x70DB): DeviceEntry(
        vendor="Thermalright",
        product="Wonder Vision",
        model="WONDER_VISION",
        button_image="A1WONDER VISION",
...
def parse_metric_spec(
    ...
    if len(parts) < 2:
        raise ValueError(
            f"Invalid metric spec '{spec}' — expected 'key:x,y' (e.g. 'gpu_temp:10,20')"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid coordinates in '{spec}' — expected 'key:x,y' (e.g. 'gpu_temp:10,20')"
        ) from e
    ...
    if len(parts) >= 4 and parts[3]:
        try:
            ...
            raise ValueError(f"Invalid size in '{spec}' — expected integer") from e
    ...
    *,
    ...
    for i, spec in enumerate(metrics):
        key, elem = parse_metric_spec(
            spec, i, default_color, default_font_size, default_font, default_style
        )
...
# LCD brightness levels (0-3) to percent (Windows TRCC button levels + Off)
BRIGHTNESS_LEVELS: Dict[int, int] = {0: 0, 1: 25, 2: 50, 3: 100}
...
def get_profile(fbl: int, pm: int = 0) -> DeviceProfile:
    ...
    if fbl == 224:
        ...
        return DeviceProfile(
            w, h, jpeg=profile.jpeg, big_endian=profile.big_endian, rotate=profile.rotate
        )
    if fbl == 192:
...
JPEG_MODE_FBLS: frozenset[int] = frozenset(fbl for fbl, p in FBL_PROFILES.items() if p.jpeg)
...
RESOLUTION_TO_PM: dict[tuple[int, int], int] = {
    p.resolution: fbl for fbl, p in FBL_PROFILES.items() if fbl not in (37, 101, 102, 224)
}
...
DEVICE_BUTTON_IMAGE: dict[int, dict[Optional[int], str]] = {
    ...
    4: {1: "A1HYPER VISION", 2: "A1RP130 VISION", 3: "A1LM16SE", 4: "A1LF10V", 5: "A1LM19SE"},
    ...
    32: {0: "A1ELITE VISION", 1: "A1FROZEN WARFRAME PRO", None: "A1ELITE VISION"},
    ...
    63: {0: "A1FROZEN WARFRAME PRO", 1: "A1WONDER VISION", 2: "A1LM27", 3: "A1LM30"},
    64: {0: "A1FROZEN WARFRAME PRO", 1: "A1WONDER VISION", 2: "A1LM27", 3: "A1LM30"},
    65: {0: "A1ELITE VISION", 1: "A1LF14", 3: "A1LD7", 4: "A1LD10", 5: "A1LD7"},
    66: {0: "A1ELITE VISION", 1: "A1LF14", 2: "A1LF14", 3: "A1LD7", 4: "A1LD7"},
    68: {None: "A1TROFEO VISION"},
    69: {2: "A1LD9"},
    100: {0: "A1FROZEN WARFRAME PRO", 1: "A1WONDER VISION", None: "A1FROZEN WARFRAME PRO"},
    101: {0: "A1ELITE VISION", 1: "A1LF14", None: "A1ELITE VISION"},
    128: {None: "A1TROFEO VISION"},
    129: {None: "A1GRAND VISION"},
    # -- SCSI devices (VID → {PID: image}) --
    0x87CD: {0x70DB: "A1TARAN ARMS", None: "A1TARAN ARMS"},
    0x87AD: {0x70DB: "A1WONDER VISION", None: "A1WONDER VISION"},
    0x0402: {0x3922: "A1FROZEN WARFRAME", None: "A1FROZEN WARFRAME"},
    0x0416: {0x5406: "A1TARAN ARMS", None: "A1TARAN ARMS"},
}
...
PROTOCOL_TRAITS: Dict[str, ProtocolTraits] = {
    ...
    "hid": ProtocolTraits(
        udev_subsystems=("hidraw", "usb"),
        backend_key="pyusb",
        fallback_backend="hidapi",
...
SENSOR_TO_OVERLAY: dict[str, tuple[int, int]] = {
    ...
    "memory_usage": (2, 2),
    "memory_clock": (2, 3),
    "memory_available": (2, 4),
    "hdd_temp": (3, 1),
    "hdd_activity": (3, 2),
    "hdd_read": (3, 3),
    "hdd_write": (3, 4),
    "network_upload": (4, 1),
    "network_download": (4, 2),
    "network_total_up": (4, 3),
    "network_total_dl": (4, 4),
    "fan_cpu_fan": (5, 1),
    "fan_gpu_fan": (5, 2),
    "fan_ssd_fan": (5, 3),
    "fan_fan2": (5, 4),
